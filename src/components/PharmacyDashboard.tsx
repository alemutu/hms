...
export const PharmacyDashboard = () => {
    ...
    try {
      ...
      const billingItems = prescription.medications.map(med => {
        ...
        return {
          ...
          unitPrice: stock?.price || 200, // Use stock price or default
          totalAmount: (stock?.price || 200) * med.quantity,
          department: 'pharmacy',
          category: 'medication',
          status: 'pending',
  ...
  const handlePatientSelect = (patientId: string) => {
    // Find the patient's prescription
    const patientPrescriptions = prescriptions[patientId] || [];
    const pendingPrescription = patientPrescriptions.find(p => 
      p.status === 'pending' || p.status === 'stock-verified'
    );
    
    if (pendingPrescription) {
      setSelectedPrescription(pendingPrescription.id);
                ...
                {viewMode === 'grid' ? (
                  <div className="p-4 grid grid-cols-2 gap-4">
                    {activeTab === 'inventory' ? (
                      ...
                      filteredPrescriptions.map(({ patient, prescription }) => (
