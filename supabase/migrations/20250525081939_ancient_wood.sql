      ...
      USING (
        auth.uid() IN (
          ...
          WHERE 
            ...
            (users_1.hospital_id IS NULL OR users_1.hospital_id = users.hospital_id)
        )
      );
  END IF;
END $$;
...
BEGIN
  ...
  ELSE
    ...
    -- Fix the ambiguous column reference by using a table alias
    UPDATE public.users
    SET 
      role = 'super_admin',
      status = 'active',
      hospital_id = hospital_id_var
    FROM (SELECT id AS hospital_id_var FROM public.hospitals LIMIT 1) h
